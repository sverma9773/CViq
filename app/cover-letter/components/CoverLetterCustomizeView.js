...
const SIZE_OPTIONS = {
  ...
  bodySize: ["9.5pt", "10pt", "10.5pt", "11pt", "11.5pt", "12pt"],
};
...
function AdvancedCustomization() {
  ...
  const tpl = getTemplate(coverLetterData.template || "classic");
  ...
  return (
    <div className="adv">
      <div className="adv__header">
        <h3 className="adv__title">Advanced Styling</h3>
        <button className="adv__reset" onClick={resetAll}>Reset Defaults</button>
      </div>
      ...
      <div className="adv__group">
        ...
        <span className="adv__hint">All fonts are ATS-approved and print-ready</span>
      </div>
      ...
      <div className="adv__group">
        <label className="adv__label">Body text Size</label>
        <div className="adv__pills">
...
export default function CoverLetterCustomizeView() {
  const { coverLetterData, dispatch } = useCoverLetter();
  const template = coverLetterData.template || "classic";
    ...
    <div className="customize-view">
      ...
      <style jsx>{`
        ...
        @media (max-width: 768px) {
          .customize-view { flex-direction: column; }
          .customize-view__left { width: 100%; min-width: 0; border-right: none; border-bottom: 1px solid var(--color-border); max-height: 50vh; }
        }
      `}</style>
    </div>
  );
